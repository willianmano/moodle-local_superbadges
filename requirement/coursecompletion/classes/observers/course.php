...
namespace superbadgesrequirement_coursecompletion\observers;
 ...
 */
class course {
    public static function completed(\core\event\course_completed $event) {
        global $DB;

        $requirements = $DB->get_records('local_superbadges_requirements', [
            'method' => 'coursecompletion', 'target' => $event->courseid
        ]);
